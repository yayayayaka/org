...
They are not managed automatically, but can easily be imported by repository admins and org owners.
...
While the rulesets endpoint can be read by non-owners, the export must be done by a repository admin or org owner to include the `bypass_actors` field.
